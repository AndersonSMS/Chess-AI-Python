...
class Board:
    ...
    def iniciar(self, board):
        board.empty = [[None for x in range(8)] for y in range(8)]
        board.black_king = King("b", 0, 4) # these objects are stored for easy
        board.white_king = King("w", 7, 4) # access for special move checking
        board.white_rook_left = Rook("w", 7, 0)
        board.white_rook_right =  Rook("w", 7, 7)
        board.black_rook_left = Rook("b", 0, 0)
        board.black_rook_right = Rook("b", 0, 7)
        board.array = [
            [board.black_rook_left, Knight("b", 0, 1), Bishop("b", 0, 2), Queen("b", 0, 3),
                board.black_king, Bishop("b", 0, 5), Knight("b", 0, 6), board.black_rook_right],
            [Pawn("b", 1, i) for i in range(8)],
            [None for x in range(8)],
            [None for x in range(8)],
            [None for x in range(8)],
            [None for x in range(8)],
            [Pawn("w", 6, i) for i in range(8)],
            [board.white_rook_left, Knight("w", 7, 1), Bishop("w", 7, 2), Queen("w", 7, 3),
                board.white_king, Bishop("w", 7, 5), Knight("w", 7, 6), board.white_rook_right]
        ]
        # used in minimax to find the best move
        board.score = 0
        # maps piecetype to relative value
        board.pvalue_dict = {King: 200, Queen: 10,
                            Rook: 5, Knight: 3, Bishop: 3, Pawn: 1}
